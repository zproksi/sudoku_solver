...
IF NOT EXIST "%CMAKECMD%" (
    ...
    echo ----------------------------------
    echo  Rebuilding of solver %MODE% mode
    echo ----------------------------------
)
...
set "FILENAME=%PROJECTDIR%buildWin%MODE%\srcsolver\%MODE%\solver.exe"

IF EXIST "%FILENAME%" (
  echo   solver.exe built for %MODE% mode is here:
  echo %FILENAME%
) ELSE (
  echo   solver.exe built for %MODE% mode could not be found at
  echo %FILENAME%
)
