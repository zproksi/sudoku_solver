...
FILENAME="$PROJECTDIR/buildLinux${MODE}/srcsolver/solver"

if [ -e "${FILENAME}" ]; then
    echo "solver built for ${MODE} mode is here:"
    echo "${FILENAME}"
else
    echo -e "\033[31mERROR\033[0m solver built for ${MODE} mode could not be found at"
    echo "${FILENAME}"
fi
